...
class rabi_pulse(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_loop(name="gain_loop", count=cfg.expt.steps)
...
class rabi_gain(Experiment):
    def __init__(self, path='', prefix='rabi_gain', config_file=None, liveplot_enabled=True, **kwargs):
    ...
    def acquire(self, progress=False):
        ...
        gs = np.linspace(self.cfg.expt.min_gain, self.cfg.expt.max_gain, self.cfg.expt.steps) # relaxation time should be somewhere else in the cfg
        iq_list = prog.acquire(self.soc)
        self.data = {"I": iq_list[0][0][:,0], "Q": iq_list[0][0][:,1], "gs": gs}
        return self.data
    ...
    def display(self, save=True):
        ...
        plt.plot(gs, mag, '-o', label="Magnitude")
        plt.title("Gain Rabi")
        plt.xlabel("Gain")
