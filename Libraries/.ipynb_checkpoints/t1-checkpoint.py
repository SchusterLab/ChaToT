...
class t1_pulse(AveragerProgramV2):
    ...
    def _body(self, cfg):
        ...
        self.pulse(ch=cfg.soc.res_gen_ch, name="res_pulse", t=cfg.expt.delay+cfg.pulses.pi_const.length+0.01)
        self.trigger(ros=[cfg.soc.ro_ch], pins=[0], t=cfg.expt.trig_offset+cfg.pulses.pi_const.length+cfg.expt.delay, ddr4=False)


class t1(Experiment):
    def __init__(self, path='', prefix='t1', config_file=None, liveplot_enabled=True, **kwargs):
        super().__init__(path=path, prefix=prefix, config_file=config_file, liveplot_enabled=liveplot_enabled, **kwargs)
        return

    def acquire(self, progress=False):
        super().acquire()
        prog = t1_pulse(soccfg=self.soccfg, reps=self.cfg.expt.n_avg, final_delay=self.cfg.expt.relaxation_time, cfg=self.cfg)
